...
notebook_str = r'''
{
 "cells": [
  ...
  },
  {
   ...
   "source": [
    "%%time\n",
    "marked=nrt.mark_weird(df, select_func=lambda x: nrt.select_after_changelog(x, changelog, nmin=8), naive=False, confidence=0.99, col=factor)\n",
    "nb_weird = len(marked[marked.weird == True])\n",
